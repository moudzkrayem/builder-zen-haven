/**
 * Simple emulator test using @firebase/rules-unit-testing
 *
 * Usage:
 * 1. Install deps locally: npm i -D @firebase/rules-unit-testing
 * 2. Start Firestore emulator in another shell (Firebase CLI must be installed):
 *    firebase emulators:start --only firestore --project=trybe-2d135
 * 3. Run this script with node:
 *    node tests/emulator-firestore-test.js
 *
 * The script will load rules from ../firestore.rules and run a few checks:
 * - Alice (attendee) can create/read messages
 * - Bob (not attendee) cannot read/create messages until added to attendeeIds
 */
...
(async () => {
  ...
  try {
    ...
    // As admin (unauthenticated testEnv.unauthenticatedContext? use admin) create a trybe doc via privileged context
